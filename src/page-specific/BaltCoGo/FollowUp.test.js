...
test('should display the service request when a valid service request is searched', async () => {
	...
	await wait(() => {
		...
		const reportContainerElement = appContainer.getElementsByClassName('bc-citysourced-reporter');
...
test('should display a link when a user enters a animal service request id', () => {
	...
	const differentTrackingMessage = getByText(alertPanel, /.*in a different tracking system/i);
